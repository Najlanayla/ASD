...
class DoubleLinkedList:
    ...
    def jumpsearchNama(self, nama_obat):
        current = self.head
        n = self.size
        step = int(n ** 0.5)
        prev = None
        while current and current.nama_obat < nama_obat:
            prev = current
            for _ in range(step):
                if current.next:
                    current = current.next
        ...
        while prev and prev.nama_obat < nama_obat:
            prev = prev.next

        if prev and prev.nama_obat == nama_obat:
            return prev
        else:
            return None

    def jumpsearchHarga(self, harga_obat):
        current = self.head
        n = self.size
        step = int(n ** 0.5)
        prev = None
        while current and current.harga_obat < harga_obat:
            prev = current
            for _ in range(step):
                if current.next:
                    current = current.next
        ...
        while prev and prev.harga_obat < harga_obat:
            prev = prev.next

        if prev and prev.harga_obat == harga_obat:
            return prev
        else:
            return None
...
data.add("Paracetamol", "500 mg", 100, 15000, "01/01/2025")
data.add("Amoxicillin", "250 mg", 50, 7000, "19/02/2025")
data.add("Vitamin C", "20 mg", 75, 10000, "25/03/2025")
data.add("Aspirin", "100 mg", 30, 6000, "20/12/2024")
data.add("Ibuprofen", "400 mg", 90, 5500, "11/07/2024")
...
|       4. Hapus Data Obat         |
|       5. Urut Data Obat          |   
|       6. Pencarian Data Obat     |
|       7. Keluar                  |
====================================
    Masukan pilihan (1/2/3/4/5/6/7):  """))
    ...
    if menu == 1:
        ...
        while ulang == "y":
            ...
            while True:
                exp = input("Masukkan tanggal EXP date (format: DD/MM/YYYY): ")
                if exp.strip():
                    try:
                        datetime.strptime(exp, "%d/%m/%Y")
                        break
                    except ValueError:
...
|      Pencarian Data Obat      |
=================================""")  
        print("  [1] Pencarian data berdasarkan Nama")
        print("  [2] Pencarian data berdasarkan Harga")
